...
interface ImmuneFromTabsParams {
  tabs: TreeNode[];
  tabsInScope: NodeValue[];
}

function getImmuneChartIdsFromTabsNotInScope({
  tabs = [],
  tabsInScope = [],
}: ImmuneFromTabsParams): NodeValue[] {
  const chartsNotInScope: TreeNode[] = [];
...
interface TabChildrenScopeParams {
  tabScopes: Record<string, FilterScope>;
  parentNodeValue: NodeValue;
  forceAggregate?: boolean;
  hasChartSiblings?: boolean;
  tabChildren?: TreeNode[];
  immuneChartSiblings?: NodeValue[];
}
...
function getTabChildrenScope({
  ...
  tabChildren = [],
  immuneChartSiblings = [],
}: TabChildrenScopeParams): FilterScope {
...
function traverse({
  ...
  const chartsImmune = chartChildren
    ...
    .map(({ value }) => value);
  const tabScopes: Record<string, FilterScope> = flow(
    keyBy((child: TreeNode) => child.value),
    mapValues((child: TreeNode) =>
      traverse({
        currentNode: child,
        filterId,
        checkedChartIds,
...
interface GetFilterScopeFromNodesTreeParams {
  filterKey: string;
  nodes?: TreeNode[];
  checkedChartIds?: NodeValue[];
}
...
export default function getFilterScopeFromNodesTree({
  ...
  nodes = [],
  checkedChartIds = [],
}: GetFilterScopeFromNodesTreeParams): FilterScope {
  if (nodes.length) {
